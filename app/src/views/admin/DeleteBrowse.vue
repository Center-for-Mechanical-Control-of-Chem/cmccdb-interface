...
export default {
  ...
  methods: {
    ...
    deleteDataset(id) {
        if (id in this.deleteConfirmations) {
            const db = this.getDBQuery()
            fetch(`/api/delete/${id}`+db, {method: "POST"})
            ...
            alert("this will permanently delete this dataset, click again only if you are sure you want to delete");
            this.deleteConfirmations[id] = true
        }
    },
    resetDatabase() {
        if (this.resetConfirmationCounter < 1) {
            alert("This will reset this database, are you sure you want to do that?");
            this.resetConfirmationCounter += 1
        } else if (this.resetConfirmationCounter < 2) {
            alert("Are you really sure? This is your last warning");
            this.resetConfirmationCounter += 1
        } else {
            const db = this.getDBQuery()
            fetch("/api/reconfigure"+db, {method: "POST"})
            .then(response => response.json())
            .then(data => {
                if ("traceback" in data) {
                    this.traceback = "ERROR: \n" + data["traceback"]
    ...
    .table-container
      ...
      template(
        ...
        .column 
          ...
          button.material-symbols-outlined.btn.btn-secondary.p-1.rounded-circle.delete-icon(
            @click="deleteDataset(row[\"Dataset ID\"])"
          ) close
        .column {{row.Name}}
